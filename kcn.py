    ...
    def forward(self, coords, features, train_indices=None):
        ...
        if train_indices is not None:
            ...
            for i in range(len(coords)):
                neighbors = self.knn.kneighbors(coords[i:i+1], return_distance=False)
                with torch.no_grad():
                    att_graph = self.form_input_graph(coords[i], features[i], neighbors[0])
                    batch_inputs = [att_graph]

                batch_inputs = self.collate_fn(batch_inputs).to(self.device)
                output = self.gnn(batch_inputs.x, batch_inputs.edge_index, batch_inputs.edge_attr)
                output = torch.reshape(output, [-1, (self.n_neighbors + 1), output.shape[1]])
                center_output = output[:, 0]
                new_preds = self.last_activation(self.linear(center_output)).cpu().detach().numpy()

                # 将当前预测结果添加到训练集中
                new_coords = coords[i:i + 1].cpu().numpy()
                new_features = features[i:i + 1].cpu().numpy()
                pred.append(new_preds)

                # 扩展训练集
                # self.trainset.coords = torch.cat([self.trainset.coords, new_coords], dim=0)
                # self.trainset.features = torch.cat([self.trainset.features, new_features], dim=0)
                # self.trainset.y = torch.cat([self.trainset.y, new_preds], dim=0)
                self.trainset.coords = torch.tensor(np.concatenate([self.trainset.coords, new_coords], axis=0))
                self.trainset.features = torch.tensor(np.concatenate([self.trainset.features, new_features], axis=0))
                self.trainset.y = torch.tensor(np.concatenate([self.trainset.y, new_preds], axis=0))

                self.knn.fit(self.trainset.coords)

            pred = torch.tensor(np.array(pred), dtype=torch.float32).to(self.device)
            pred = pred.squeeze(dim=2)  # 去掉维度为1的第三维
